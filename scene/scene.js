...
function render() {
  ...
  if (moved === true && user.myTurn === true) {
    ...
    displayMessage('Height: ' + parseFloat(ball.position.y - .3).toFixed(3), 'Distance: ' + parseFloat(5 - ball.position.x).toFixed(3));
  ...
  if (delayedTrackerTime.counter >= 10 && delayedTrackerTime.flag === true){
    ...
    ball.setLinearVelocity(new THREE.Vector3(-3, 5, 0));
  ...
  if (delayedTrackerTime.counter > 3 && delayedTrackerTime.counter < 5 && delayedTrackerTime.flag === true){
    ...
    ball.setLinearVelocity(new THREE.Vector3(-2, 18, 0));
    delayedTrackerTime.flag = false;
    delayedTrackerTime.counter = 0;
    moved = true;
    dataChannel.send(JSON.stringify({ 'moved': moved }));
...
if (user.player === "user_1") {
  var pointsDiv = $( "<div id='pointsDiv'>Player 1 Points: <span id ='p1Points'>" + user.points + "</span><br>Player 2 Points: <span id='p2Points'>" + otherUser.points + " </span> </div>" );
} else {
  var pointsDiv = $( "<div id='pointsDiv'>Player 1 Points: <span id ='p1Points'>" + otherUser.points + "</span><br>Player 2 Points: <span id='p2Points'>" + user.points + " </span> </div>" );
}
$('#bg').append(pointsDiv);
$('#bg').append(throwBall);
$('#pointsDiv').fadeOut(0);
